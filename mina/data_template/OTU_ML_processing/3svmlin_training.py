...
parser.add_argument('DATAFILE', type=str, help='Training datafile (X\'s)')
parser.add_argument('SAMPLE_NAMES', type=str, help='Names (or IDs) of samples')
parser.add_argument('VAR_NAMES', type=str, help='Names (or IDs) of variables (e.g. bacteria)')
...
BASEFILE = os.path.splitext(os.path.basename(XDATAFILE))[0]
...
x = np.loadtxt(XDATAFILE, dtype=float, delimiter='\t')
sample_names = np.loadtxt(SAMPLE_NAMES, dtype=str, delimiter='\t')
var_names = np.loadtxt(VAR_NAMES, dtype=str, delimiter='\t')
...
print "sample_names"
print sample_names
#print "sample_names size", sample_names.shape
print "var_names"
print var_names
#print "var_names size", var_names.shape
"""
...
log_w.writerow(["Data file", XDATAFILE])
